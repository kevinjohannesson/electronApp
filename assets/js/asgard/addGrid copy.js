...
function normalize(min = 100, max = 10) {
  // Calculates the normalized position of 0 between min and max.
  //  If 0 is not present, normalizes the position of the lowest integer
  let n; 
  if (min > max) { 
    if (min > 0 && max < 0) {
      n = lerp( 0, max, min, 0, 1 ) } 
    else { n = 1 } } 
  else {
    if (min < 0 && max > 0) {
      n = lerp( 0, min, max, 0, 1  ) } 
    else { n = 0 } }
...
class CoordinatePlane {
  constructor() {
    ...
    this.grid = { 
      axes: { x: new Axis(-10, 10), y: new Axis(-10, -15) },
    ...
    xAxis.constant = this.getAxisConstantValue( "x", yAxis.min, yAxis.max );
    ...
    xAxis.numbers = this.addNumbers()
    yAxis.numbers = this.addNumbers('y')
  }

  addNumbers( axis = 'x' ) {
    let { min, max } = this.grid.axes[axis]
  ...
  createGridLines(axisLetter = "x", gridLineEvery = 1) {
    ...
    const { length } = this.grid.axes[ is_xAxis ? "y" : "x" ]
